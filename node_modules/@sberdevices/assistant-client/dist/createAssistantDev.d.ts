...
export declare const createAssistantDev: <A extends AssistantSmartAppData>({ getState, getRecoveryState, ready, ...sdkParams }: CreateAssistantParams & Pick<InitializeAssistantSDKParams, 'initPhrase' | 'url' | 'userChannel' | 'surface' | 'userId' | 'token' | 'surfaceVersion' | 'nativePanel' | 'sdkVersion' | 'enableRecord' | 'recordParams' | 'fakeVps' | 'settings' | 'getMeta' | 'features'>) => {
    nativePanel: {
        hide: () => void;
        show: () => void;
    };
    close: () => void;
    getInitialData: () => import("./typings").AssistantClientCustomizedCommand<import("./typings").AssistantSmartAppCommand>[];
    findInInitialData: <T>({ type, command }: {
        type?: string | undefined;
        command?: string | undefined;
    }) => T | undefined;
    getRecoveryState: () => unknown;
    on: <K extends "error" | "command" | "data" | "start">(event: K, cb: import("./createAssistant").AssistantEvents<A>[K]) => () => void;
...
export declare const createSmartappDebugger: <A extends AssistantSmartAppData>({ token, getState, getRecoveryState, ready, settings, nativePanel, ...sdkParams }: {
    token: string;
    settings?: Pick<AssistantSettings, "dubbing"> | undefined;
} & CreateAssistantParams & Pick<InitializeAssistantSDKParams, "nativePanel"> & Pick<InitializeAssistantSDKParams, "getMeta" | "initPhrase" | "enableRecord" | "recordParams">) => {
    nativePanel: {
        hide: () => void;
        show: () => void;
    };
